...
// تحديث الأسئلة: (عربي = رمضاني) | (إنجليزي = عام وعالمي)
const questionsSets = {
  set1: {
    arabic: [
...
function App() {
  ...
  const [language, setLanguage] = useState('arabic'); // اللغة الافتراضية
  ...
  return (
    <div className="min-h-screen relative overflow-hidden text-white font-sans bg-[#0d041a]">
      ...
      <main className="relative z-10 container mx-auto p-4 pb-24 text-center">
        {activeTab === 'home' && (
          <div className="max-w-2xl mx-auto space-y-6">
            ...
            <div className="bg-white/5 p-6 rounded-2xl border border-white/10 backdrop-blur-md">
              ...
              <input type="text" value={playerName} onChange={(e) => setPlayerName(e.target.value)} placeholder={language === 'arabic' ? "سجل اسمك..." : "Enter your name..."} className="w-full bg-white/10 p-4 rounded-xl text-center text-xl outline-none mb-4 border border-white/10" />
              ...
              <p className="mt-2 text-yellow-400 font-bold">{language === 'arabic' ? `المقاعد: ${20 - players.length} / 20` : `Slots: ${20 - players.length} / 20`}</p>
            </div>
            ...
            <div className="bg-[#1a0f00] rounded-3xl p-8 border-2 border-yellow-600 shadow-[0_0_30px_rgba(234,179,8,0.3)]">
              ...
              <p className="text-3xl mt-4 font-bold text-white uppercase tracking-wider">
                {leagueWinner ? `🐐 ${leagueWinner} 🐐` : (language === 'arabic' ? "⏳ في انتظار البطل" : "⏳ Waiting for Hero")}
              </p>
            </div>
          </div>
        )}

        {/* باقي التبويبات بدون أي تغيير في التصميم */}
        ...
        {/* التبويبات الأخرى تبقى كما هي (Live, History, Prizes) */}
        {activeTab === 'live' && <div className="py-20 text-white/40">🔴 جاري تحديث المواجهات الحية...</div>}
        {activeTab === 'history' && <div className="py-20 text-white/40">📜 سجل الأبطال فارغ حالياً</div>}
        {activeTab === 'prizes' && (
          <div className="max-w-2xl mx-auto bg-yellow-500/10 p-8 rounded-3xl border border-yellow-500/30 text-right font-bold">
            <h2 className="text-2xl font-bold text-yellow-400 mb-6 text-center">🏆 الجوائز والقوانين</h2>
            <p className="mb-4">1. الدوري ينطلق عند اكتمال 20 لاعباً.</p>
